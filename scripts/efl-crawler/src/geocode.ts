...
/** Viti Levu west/central — avoid matching "Ba Island" or other islets */
...
/**
 * Mainland division anchors (Ba Town, Nadi, etc.)
 * Used when Nominatim only finds "Ba" → wrong island.
 */
...
/** EFL place names → approximate coords on Viti Levu / Vanua Levu */
...
/** Reject "Ba Island" and other wrong matches east of mainland */
function isBadBaMatch(lat: number, lng: number, division: string | null): boolean {
  if (division !== "Ba") return false;
  // Ba Island / offshore dots are often lng > 178.2 with lat around -17.1
...
export function buildGeocodeQueries(region: string, area: string): string[] {
  ...
  const division = extractDivision(regionClean);

  // Most specific: full region on Viti Levu (Ba province is on main island, NOT Ba Island)
  if (regionClean) {
  ...
  if (parts.length >= 2) {
    ...
    // Never use bare "Ba, Fiji" — hits the wrong island
...
export async function geocodeFijiWithFallback(
  ...
  // 1. Known EFL place names (Qerelevu, etc.)
  ...
  // 2. Nominatim with mainland bias for Viti Levu divisions
  ...
  // 3. Division anchor (e.g. Ba Town) — better than wrong island
